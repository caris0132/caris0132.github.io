 ...
 *
 */
class Ifdb_Validator_String extends Ifdb_Validator_Base
{
    ...
    public function validate()
    {
        ...
        if (function_exists('mg_strlen') && isset($this->config['encoding'])) {
        ...
        if (isset($this->config['maxLength']) && $length > $this->config['maxLength']) {
            $this->setError('Field "' . $this->config['label'] . '" is too long (maxnimum is ' . $this->config['maxLength'] . ' characters).');
        }
